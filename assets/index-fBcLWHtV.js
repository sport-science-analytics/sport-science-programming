...
distance_m = 2900
vo2max = (distance_m - 504.9) / 44.73
print(f"Estimated VO2max: {vo2max:.1f} mL/kg/min")`},{type:"exercise",id:"ex-4-20",title:"Fix the Typo in VO2max",domain:"physiology",description:"The code below crashes with a NameError because of a single typo. Read the error message carefully -- Python even tells you what it thinks you meant. Fix the typo so the code prints the correct VO2max.",initialCode:`distance_m = 3050
vo2max = (distance_m - 504.9) / 44.73
print(f"VO2max: {voo2max:.1f} mL/kg/min")`,expectedOutput:"VO2max: 56.9 mL/kg/min",hints:["Read the traceback's last line -- it names the undefined variable and suggests what you meant.",'print(f"VO2max: {___:.1f} mL/kg/min")']},{type:"md",md:`---
...
## Errors in This Course's Editor

When you run code in this course's editor and it fails, the **output panel** shows you exactly the same traceback you have just learned to read -- the error type, the message, and the line number. There is no difference between an error here and an error in a "real" Python setup; it is the same Python underneath, reporting in the same way.

So when the output panel turns red, do not skim past it. Read the last line for the type and message, find the line number, and compare against your code -- the very habit this lesson has drilled.

## Summary

- A **traceback** is Python's map to a problem, not a telling-off. Reading them is a daily activity for every programmer.
- **Read from the bottom up:** last line first (type and message), then the line number, then climb only if you must.
- **SyntaxError** and **IndentationError** are caught **before anything runs**; **NameError**, **TypeError**, **ValueError**, **IndexError**, and **KeyError** strike while the program runs.
- **Logic errors** produce no traceback at all: the program runs cleanly and hands you a wrong answer.
...
# Code agrees -- this piece is correct`},{type:"exercise",id:"ex-4-40",title:"Find the Logic Error",domain:"physiology",description:"This code calculates training intensity as a fraction of maximum heart rate, but the formula is upside down -- it divides the maximum by the session heart rate. The result should be below 1 (the session HR is below the max). Fix the formula so it prints correctly.",initialCode:`hr_bpm = 155        # session average heart rate
hr_max_bpm = 198    # tested maximum heart rate

# intensity = session HR / max HR -- but the formula below is wrong
intensity = hr_max_bpm / hr_bpm
print(f"Intensity: {intensity:.2f}")`,expectedOutput:"Intensity: 0.78",hints:["A fraction of max must be below 1 here -- the smaller number (the session HR) goes on top.",`intensity = hr_bpm / ___
print(f"Intensity: {intensity:.2f}")`]},{type:"md",md:`---
...
This six-step routine is the whole lesson in miniature. Follow it in order and debugging stops being magic and becomes method. One last exercise to put it to work:`},{type:"exercise",id:"ex-4-43",title:"Trace and Fix a Unit Error",domain:"biomechanics",description:"Jump power is calculated as force x velocity, with velocity in m/s. The velocity below is in km/h but is used directly, so the power comes out inflated (2125.0 W). Add the unit conversion (divide by 3.6) so the power prints correctly.",initialCode:`force_n = 850        # newtons
velocity_kmh = 2.5   # km/h -- the formula needs m/s

power_w = force_n * velocity_kmh
print(f"Power: {power_w:.1f} W")`,expectedOutput:"Power: 590.3 W",hints:["Convert first: velocity in m/s = velocity in km/h divided by 3.6. Then multiply by the force.",`velocity_ms = velocity_kmh / ___
power_w = force_n * velocity_ms
print(f"Power: {power_w:.1f} W")`]},{type:"md",md:`---

## Summary

- **Debugging is detective work:** the bug is always explainable, and **systematic beats guessing.** The hardest bugs are logic errors -- the code runs with no traceback but gives a wrong answer.
- **Read the error first** (bottom up) whenever there is a traceback -- it solves most bugs immediately.
- **Print debugging is the workhorse:** scatter \`print()\` checkpoints, watch the intermediate values, and find the first one that is wrong.
- **Isolate the problem:** comment out pieces and run smaller chunks; test with **tiny data you can verify by hand**.
- **Explain it out loud** (rubber-duck debugging) -- saying the code aloud forces the precision that finds the bug.
- **AI-assisted debugging:** paste the full traceback, the relevant code, and expected-vs-got; then **verify the suggested fix** by running it and checking the number makes physical sense.
- Follow the **6-step checklist:** read the error, reproduce, isolate, inspect with prints, form a hypothesis, fix and re-run.
